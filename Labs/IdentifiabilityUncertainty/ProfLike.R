...
ProfLike = function(params,profindex,costfun,times,data,perrange=0.5,numpoints=10){
  ...
  currparams = matrix(2*numpoints,length(params))
  ...
  for (i in 1:2){
    ...
    for (j in 1:numpoints){
      ...
      currfvals[j+length(numpoints)*(i-1)] = res$value
      currflags[j+length(numpoints)*(i-1)] = res$convergence
      currparams[,j+length(numpoints)*(i-1)] = append(res$par,profrange[j,i],after=profindex-1);  #recording the  full set of params (even though it includes the profparam), to make it easier to just run the model with the profile output after ProfLike is done (i.e. you don't have to recombine the profiled param with the rest to run the model)
    }
  }
