...
Methods marked in italics are included in the `BayesianTools` package, but I've included a couple of other common ones since they're in commonly used packages like `Stan` or `POMP`. Links to more info included as well (also more info [here](https://cran.r-project.org/web/packages/BayesianTools/vignettes/BayesianTools.html))!
...
- *Differential evolution* - runs multiple chains in parallel, and the proposal distribution depends on the position of all the chains (see [ter Braak 2006](https://link.springer.com/article/10.1007/s11222-006-8769-1) and [ter Braak and Vrugt 2008](https://link.springer.com/content/pdf/10.1007/s11222-008-9104-9.pdf))
- [Hamiltonian methods](https://en.wikipedia.org/wiki/Hamiltonian_Monte_Carlo) (Stan) - uses a Hamiltonian approach to generate distant samples that are more likely to be accepted (nice blog post about this [here](http://arogozhnikov.github.io/2016/12/19/markov_chain_monte_carlo.html))
- [Particle filtering](https://kingaa.github.io/pomp/) (in particular, [Iterated Filtering](https://en.wikipedia.org/wiki/Iterated_filtering) in [POMP](https://kingaa.github.io/pomp/)) - a set of particles (points/samples in parameter space) is weighted by its likelihood or posterior, and then resampled or new particles drawn according to the weights.
